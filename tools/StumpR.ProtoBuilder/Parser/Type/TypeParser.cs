...
namespace StumpR.ProtoBuilder.Parser.Type;
...
public class TypeParser : IParser
{
    ...
    private TypeParser(RegexCache regexCache, IConfigurationRoot configuration, PrimitiveTypeCache primitiveTypeCache)
    {
    ...
    public TypeParser() : this(RegexCache.Instance, ConfigurationCache.Instance.Configuration,
        PrimitiveTypeCache.Instance)
    { }

    public ParserTypeEnum ParserType => ParserTypeEnum.Type;
    ...
    public TemplateInfo Parse(string filePath)
    {
        ...
        classInfoBuilder.SetNamespace($"{_configuration["NamespacesRoot:Types"]}");

        IList<string> directives = _configuration
            .GetSection("Directives:Types")
            .Get<List<string>>();
