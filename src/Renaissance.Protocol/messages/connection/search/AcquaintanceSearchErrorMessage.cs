//-------------------------------------------------------------------------------
// <auto-generated>
//	This code was generated by a tool.
//  Generated on 07/11/2019 23:09:08.
//	Changes to this file may cause incorrect behavior !
//  Author : Scheduler.
// </auto-generated>
//-------------------------------------------------------------------------------

using    Renaissance.Binary;
using    Renaissance.Binary.Definition;

namespace Renaissance.Protocol.messages.connection.search
{
	public class AcquaintanceSearchErrorMessage : IDofusMessage
	{
		public  const int NetworkId = 6143;
		public  int ProtocolId { get { return NetworkId; } }

		public byte Reason { get; set; }


		public AcquaintanceSearchErrorMessage() { }


		public AcquaintanceSearchErrorMessage InitAcquaintanceSearchErrorMessage(byte _reason)
		{

			this.Reason = _reason;

			return this;
		}

		public  byte[] Serialize()
		{

			using DofusWriter writer = new DofusWriter();

			writer.Write(this.Reason);

			return writer.Data;
		}

		public  void Deserialize(DofusReader reader)
		{

			this.Reason = reader.Read<byte>();

		}


	}
}
