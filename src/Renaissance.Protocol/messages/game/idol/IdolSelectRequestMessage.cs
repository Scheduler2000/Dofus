//-------------------------------------------------------------------------------
// <auto-generated>
//	This code was generated by a tool.
//  Generated on 07/11/2019 23:09:12.
//	Changes to this file may cause incorrect behavior !
//  Author : Scheduler.
// </auto-generated>
//-------------------------------------------------------------------------------

using    Renaissance.Binary;
using    Renaissance.Binary.Definition;

namespace Renaissance.Protocol.messages.game.idol
{
	public class IdolSelectRequestMessage : IDofusMessage
	{
		public  const int NetworkId = 6587;
		public  int ProtocolId { get { return NetworkId; } }

		public CustomVar<short> IdolId { get; set; }

		public WrappedBool Activate { get; set; }

		public WrappedBool Party { get; set; }


		public IdolSelectRequestMessage() { }


		public IdolSelectRequestMessage InitIdolSelectRequestMessage(CustomVar<short> _idolid, WrappedBool _activate, WrappedBool _party)
		{

			this.IdolId = _idolid;
			this.Activate = _activate;
			this.Party = _party;

			return this;
		}

		public  byte[] Serialize()
		{

			using DofusWriter writer = new DofusWriter();

			byte box = 0;
			box = writer.SetFlag(box,0,this.Activate);
			box = writer.SetFlag(box,1,this.Party);
			writer.Write(box);
			writer.Write(this.IdolId);

			return writer.Data;
		}

		public  void Deserialize(DofusReader reader)
		{

			byte box = reader.Read<byte>();
			this.Activate = reader.ReadFlag(box,0);
			this.Party = reader.ReadFlag(box,1);
			this.IdolId = reader.Read<CustomVar<short>>();

		}


	}
}
